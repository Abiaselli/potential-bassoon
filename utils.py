...
def check_min_max_valid(min_val: torch.Tensor, max_val: torch.Tensor) -> bool:
    """
    Checks if the given minimum and maximum values are valid. Valid means:
    - They exist and are not empty.
    - Neither contains NaNs or infinite values.
    - Every element in `min_val` is less than or equal to the corresponding element in `max_val`.
    """
    # Check if tensors are empty
    if min_val.numel() == 0 or max_val.numel() == 0:
        warnings.warn(
            "Observer must be run before calling calculate_qparams. Returning default values."
        )
        return False

    # Check for NaNs
    if torch.isnan(min_val).any() or torch.isnan(max_val).any():
        print(f"Invalid quantization range: min={min_val}, max={max_val} (NaNs present)")
        return False

    # Check for infinities
    if torch.isinf(min_val).any() or torch.isinf(max_val).any():
        print(f"Invalid quantization range: min={min_val}, max={max_val} (Infinities present)")
        return False

    # Check if all elements in min_val are less than or equal to max_val
    if not torch.all(min_val <= max_val):
        print(f"Invalid quantization range: min={min_val}, max={max_val} (min > max)")
        return False
